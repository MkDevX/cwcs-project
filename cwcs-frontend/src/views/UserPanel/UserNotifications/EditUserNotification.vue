...
export default {
  ...
  computed: {
    ...
    emptyCategory() { return this.categoryId ? this.categoryId.length < 1 : true },
  ...
  methods: {
    async getNotification() {
      try {
        ...
        let { dateFaultNotice, notificationDescription, categoryId } = notification.data

        this.dateFaultNotice = new Date(dateFaultNotice)
    ...
    async editNotification() {
      try {
        this.pending = true
        await this.$axios.put(process.env.VUE_APP_API_URL + `/notifications/${this.id}`, { dateNotification: new Date(), dateFaultNotice: this.dateFaultNotice, notificationDescription: this.notificationDescription, categoryId: this.categoryId })
