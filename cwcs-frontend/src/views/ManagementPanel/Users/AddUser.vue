  ...
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between px-5 text-lg border-b border-gray-400">
      <div class="flex items-center rounded-lg py-1">
        ...
        <h1 class="text-lg text-gray-500 font-semibold uppercase">Dodaj użytkownika</h1>
      </div>
    </div>
      ...
      <div class="mt-4 w-full">
        <form>
          <div v-if="error" class="flex bg-red-500 text-white px-3 py-2 rounded relative mb-4" role="alert">
            ...
            <span class="block">{{ errorMessage ? errorMessage: 'Użytkownik nie został dodany.' }}</span>
    ...
    <div class="p-2 bg-gray-400 flex items-center justify-end">
      ...
      <button @click="addUser()" class="flex items-center justify-center bg-gray-800 cursor-pointer text-white tracking-wide py-2 px-4 w-32 rounded focus:outline-none" :class="{ 'opacity-50 cursor-not-allowed hover:bg-gray-800': !canSubmit || pending, 'hover:bg-gray-700': canSubmit && !pending }" :disabled="!canSubmit || pending" type="button">
...
export default {
  ...
  methods: {
    async addUser() {
      try {
        this.pending = true
        await this.$axios.post(process.env.VUE_APP_API_URL + '/accounts', { username: this.username, password1: this.password1, password2: this.password2, role: 'user', firstName: this.firstName, lastName: this.lastName, accessNotifications: this.accessNotifications })
