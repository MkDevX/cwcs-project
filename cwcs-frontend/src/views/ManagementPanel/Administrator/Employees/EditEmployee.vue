  ...
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between px-5 text-lg border-b border-gray-400">
      <div class="flex items-center rounded-lg py-1">
        ...
        <h1 class="text-lg text-gray-500 font-semibold uppercase">Edytuj pracownika</h1>
      </div>
    </div>
      ...
      <div class="mt-4 w-full">
        <form>
          <div v-if="error" class="flex bg-red-500 text-white px-3 py-2 rounded relative mb-4" role="alert">
            ...
            <span class="block">{{ errorMessage ? errorMessage: 'Pracownik nie został edytowany.' }}</span>
          ...
          <div class="mb-4">
            ...
            <button @click="accessNotifications = true" title="Dostęp do zarządzania zgłoszeniami(edycja, usuwanie)." class="bg-gray-400 text-white py-2 px-4 cursor-pointer focus:outline-none rounded-l-lg" :class="{ 'bg-green-700' : accessNotifications }" type="button">
              Tak
            </button>
            <button @click="accessNotifications = false" title="Brak dostępu do zarządzania zgłoszeniami(edycja, usuwanie)." class="text-white py-2 px-4 cursor-pointer focus:outline-none rounded-r-lg" :class="{ 'bg-gray-400' : accessNotifications, 'bg-red-700' : !accessNotifications }" type="button">
              Nie
            </button>
          </div>
          <div class="mb-4">
    ...
    <div class="p-2 bg-gray-400 flex items-center justify-end">
      ...
      <button @click="editEmployeeSettings()" class="flex items-center justify-center bg-gray-800 cursor-pointer text-white tracking-wide py-2 px-4 w-32 rounded focus:outline-none" :class="{ 'opacity-50 cursor-not-allowed hover:bg-gray-800': !canSubmit || pending, 'hover:bg-gray-700': canSubmit && !pending }" :disabled="!canSubmit || pending" type="button">
...
export default {
  ...
  async mounted() {
    await this.getEmployeeSettings()
  },
  ...
  methods: {
    async editEmployeeSettings() {
      try {
        ...
        var employeeSettings = {
          ...
          accessUsersManagement: this.accessUsersManagement,
          accessRoomsManagement: this.accessRoomsManagement,
          accessWorkstationsManagement: this.accessWorkstationsManagement
        }

        if(this.password1 && this.password2 && this.password1 === this.password2) {
          employeeSettings = { password1: this.password1, password2: this.password2, ...employeeSettings }
        }

        await this.$axios.put(process.env.VUE_APP_API_URL + `/accounts/employee/${this.id}`, employeeSettings)
    ...
    async getEmployeeSettings() {
      try {
        ...
        const account = await this.$axios.get(process.env.VUE_APP_API_URL + `/accounts/employee/${this.id}`)

        let { username, firstName, lastName, accessNotifications, accessUsersManagement, accessRoomsManagement, accessWorkstationsManagement } = account.data

        this.username = username
        this.firstName = firstName
        this.lastName = lastName
        this.accessNotifications = accessNotifications
        this.accessUsersManagement = accessUsersManagement
        this.accessRoomsManagement = accessRoomsManagement
        this.accessWorkstationsManagement = accessWorkstationsManagement
